...
// Released: 2015-09-15 
/* modifications:
    1- minDate and maxDate accept timestamps.
    2- not using javascript (new Date(viewValue)), as it causes timestamp-related issues. I used globalUtils.helpers.dateToObject(viewValue);
    3- add a directive datepickerToIso so the model is a string rather than a Date object
*/
angular.module('ui.bootstrap.datetimepicker', ['ui.bootstrap.dateparser', 'ui.bootstrap.position'])
    ...
    .directive('datetimePicker', ['$compile', '$parse', '$document', '$timeout', '$uibPosition', 'dateFilter', 'uibDateParser', 'uiDatetimePickerConfig', '$rootScope',
        function ($compile, $parse, $document, $timeout, $uibPosition, dateFilter, dateParser, uiDatetimePickerConfig, $rootScope) {
            return {
                restrict: 'A',
                require: 'ngModel',
                scope: {
                ...
                link: function (scope, element, attrs, ngModel) {
                    ...
                    if (attrs.timepickerOptions) {
                        ...
                        timepickerEl.attr('show-meridian', false);
                    }
                    ...
                    angular.forEach(['minMode', 'maxMode', 'minDate', 'maxDate', 'datepickerMode', 'initDate', 'shortcutPropagation'], function(key) {
                        if (attrs[key]) {
                            ...
                            scope.$parent.$watch(getAttribute, function(value) {
                                // AUTOCRUD 1 - ALTERED SO MINDATE AND MAXDATE DO NOT NEED TO BE AN ANGULAR VAR //
                                if(key == "maxDate"){
                                    scope.watchData.maxDate = new Date(parseInt(attrs[key]));
                                } else if(key == "minDate") {
                                    scope.watchData.minDate = new Date(parseInt(attrs[key]));
                                } else
                                // END AUTOCRUD //
                    ...
                    function parseDate(viewValue) {
                        ...
                        if (!viewValue) {
                            ...
                            var date = dateParser.parse(viewValue, dateFormat, scope.date) || globalUtils.helpers.dateToObject(viewValue);
                            if (isNaN(date)) {
                                return undefined;
                            } else {
                                return date;
    ...
    .directive("datepickerToIso", function(){
        return {
            restrict: "A",
            require: "ngModel",
            link: function(scope, element, attrs, ngModelCtrl){
                var auxScope = scope;
                while(auxScope.e == undefined && auxScope.c == undefined){
                    if(!!auxScope.$parent)
                        auxScope = auxScope.$parent;
                    else
                        return;
                }
                var el = !!auxScope.c ? auxScope.c : auxScope.e;
                ngModelCtrl.$parsers.push(function (datepickerValue) {
                    return globalUtils.helpers.dateToSQL(datepickerValue, el.chars.mode);
                });
            }
        };
    });
